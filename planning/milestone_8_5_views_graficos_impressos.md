...
- **F2.2c (pendente): gate Playwright.** Decisão 6 manda nascer na F2. NÃO bundlado na F2.2b de propósito: exige backend+frontend+browser juntos, e Docker/build vêm morrendo/OOMando neste ambiente — escrever E2E que não roda e dizer que passa seria afirmação-sem-prova. Fica pra rodar COM o Diretor, dois servidores de pé. Cobre o round-trip: monta gráfico no builder → publica → SVG congelado aparece no público → sobrevive ao export ZIP.
