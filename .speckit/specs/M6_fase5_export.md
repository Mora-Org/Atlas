...
4. ✅ **UI + validação** — concluído 2026-06-11: "Exportar" por card (ativa inclusa), aviso pré-geração de truncamento, "§ III Export" pós-geração detalhado. TestSprite 4/4 (export + rollback/empty/cancel re-validados em ambiente correto). **Gate `file://` cumprido** (delegado pelo Diretor a validação automatizada): Playwright abriu o ZIP exportado E o spike de 3 layouts em **Chrome, Edge e Firefox** via `file://` — fontes Fraunces/Plex carregadas (`document.fonts.check`), hero visível, zero erros de console/requests, screenshots inspecionados visualmente. Script: `frontend/scripts/validate-export-browsers.mjs`. **Resta só:** aprovação e merge do PR #28.
