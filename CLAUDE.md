...
- **M9 (Webhooks + API Keys + Audit Log):** 🔵 **F1 + F2 CODADAS.**
  ...
  - **Rota nova alcançável por key** → usar `Depends(tenant_db_principal)` + `authorize_table(...)`, nunca `tenant_db` cru: o wrapper replica o ciclo do GUC, e sem ele o FORCE RLS devolve **200 com zero linhas** em Postgres.
  - Falta **F3** (webhooks) e **F4** (fronteira) pra fechar `0.9.0`. Plano em `planning/milestone_9_webhooks_keys_audit.md`.
